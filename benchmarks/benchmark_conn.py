...
def get_blender_connection():
    ...
    if _blender_connection is not None:
        try:
            # CURRENT IMPLEMENTATION: sends a command every time
...
for _ in range(n):
    ...
    # Simulate a tool call using the connection
    # conn.send_command("get_scene_info")

end_time = time.time()
print(f"Time for {n} get_blender_connection() calls: {end_time - start_time:.4f}s")
