...
class BlenderConnection:
    ...
    def receive_full_response(self, sock, buffer_size=8192):
        ...
        # Optimization: Only attempt JSON parsing if the chunk ends with a terminator
        # This avoids quadratic overhead of repeated parsing during chunked reception
        terminators = (ord('}'), ord(']'))
        ...
        try:
            while True:
                try:
                    ...
                    # Fast check: only try parsing if the last non-whitespace character is a terminator
                    stripped_chunk = chunk.rstrip()
                    if stripped_chunk and stripped_chunk[-1] in terminators:
                        try:
                            data = b''.join(chunks)
                            # json.loads handles bytes directly in Python 3.6+
                            json.loads(data)
