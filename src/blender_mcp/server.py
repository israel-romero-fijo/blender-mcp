        ...
        try:
            while True:
                try:
                    ...
                    # Optimization: only attempt to parse if it looks like a complete JSON
                    # This avoids quadratic O(N^2) parsing attempts on large payloads
                    if chunk.rstrip().endswith((b'}', b']')):
                        try:
                            ...
                            # Incomplete JSON (e.g. nested), continue receiving
                            continue
