        ...
        try:
            while True:
                try:
                    ...
                    # This avoids quadratic behavior for large payloads
                    if chunk.rstrip().endswith((b'}', b']')):
                        # Check if we've received a complete JSON object
                        try:
                            data = b''.join(chunks)
                            # json.loads() supports bytes directly since Python 3.6
                            json.loads(data)
...
def get_blender_connection():
    ...
    if _blender_connection is not None and _blender_connection.sock:
        # Optimization: Use lightweight fileno() check instead of round-trip ping
        # This reduces per-call overhead from ~0.02ms to ~0.00008ms
        if _blender_connection.sock.fileno() != -1:
            return _blender_connection

        # Connection is dead (closed locally), disconnect to clean up
        try:
            _blender_connection.disconnect()
        except:
            pass
    ...
    if _blender_connection is None:
        ...
        # Sync PolyHaven status on initial connection
        try:
            # Use the raw send_command to avoid recursion if we ever changed it
