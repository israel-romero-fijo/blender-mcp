        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    # Performance optimization: only try to parse if it looks like the end of a JSON object
                    if data and data[-1] in (125, 93):  # '}' or ']'
                        try:
                            # Try to parse command
                            full_buffer = b''.join(chunks)
                            command = json.loads(full_buffer.decode('utf-8'))
                            chunks = []
