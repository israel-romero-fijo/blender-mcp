        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    # Optimize: only try to parse if it looks like the end of a JSON object/array
                    if data.rstrip().endswith((b"}", b"]")):
                        try:
                            # Try to parse command
                            full_data = b"".join(chunks)
                            # json.loads supports bytes directly in Python 3.6+
                            command = json.loads(full_data)
                            chunks = []

                            # Execute command in Blender's main thread
                            # Capture command via default argument to avoid closure issues
    ...
    def set_texture(self, object_name, texture_id):
        ...
        try:
            ...
            if "arm" in texture_nodes:
                ...
                # Optimize: direct boolean chain for small set check
                ...
                # Optimize: direct boolean chain for small set check
                if (
                    "metallic" not in texture_nodes
                    and "metalness" not in texture_nodes
                    and "metal" not in texture_nodes
                ):
