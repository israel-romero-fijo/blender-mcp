        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    # Performance optimization: only try to parse if the chunk looks like it might be the end
                    # of a JSON object or array. This avoids O(N^2) parsing overhead for large payloads.
                    if data.rstrip().endswith((b'}', b']')):
                        try:
                            # Try to parse command
                            full_payload = b''.join(chunks)
                            command = json.loads(full_payload.decode('utf-8'))
                            chunks = []
